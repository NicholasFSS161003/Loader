...
void LoaderV1::load(uint8_t init_dist, uint8_t rdu, uint8_t rdu_limit) {
  _isloaded = 0;
}
...
void LoaderV1::detect_LoadDist(){
  while (digitalRead(_buttonPin)==0){
    _edge_load_dist += 2;
  }
}
...
void LoaderV1::update() {
  if (_isloaded == 0) {
    if (digitalRead(_buttonPin) == 1 && _flag == 0) {
      Servo.write(init_dist - _rdu_dist);
      _rdu_dist += rdu;
      ++_counter;
      _isloaded = 1;
    } else if (digitalRead(_buttonPin) == 0 && _flag == 1) {
      _flag = 0;
    }
